...
typedef struct day{
    ...
    int id; // which day
    int *boom_root; // origin of boom
    struct day *Next_day; 
    struct day *Prev_day;
    struct day *next_boom;
    struct day *boom_tail;
}DAY;
...
int main(void){
    ...
    char cmd[10];
    int Boom[d+1]; // stores the last day of the linked list for boom
    ...
    for(int i = 0; i <= d; i++){
        ...
        Day[i].valid = 0;
        Day[i].value = 0;
        Day[i].boom_root = &Boom[i];
        Day[i].Prev_day = NULL;
        Day[i].Next_day = NULL;
        Day[i].next_boom = NULL;
        Day[i].boom_tail = &Day[i];
    }
    for(int i = 1; i <= d; i++){
        scanf("%s", cmd);
        if(cmd[0] == 'm'){
            ...
            int boom;
            scanf("%d", &boom);
            Day[i].boom_root = Day[boom].boom_root;
            Day[*Day[boom].boom_root].boom_tail -> next_boom = &Day[i];
            Day[*Day[boom].boom_root].boom_tail = &Day[i];
        }
    }
